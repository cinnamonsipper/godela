...
rv1 = multivariate_normal(mean=[0, 0], cov=[[1.0, 0.3], [0.3, 1.0]])
rv2 = multivariate_normal(mean=[1, 1], cov=[[1.5, -0.4], [-0.4, 0.8]])
rv3 = multivariate_normal(mean=[-1, 0.5], cov=[[0.8, 0.1], [0.1, 1.3]])
...
# Draw ONLY filled contours with explicit parameters to avoid lines
contourf = plt.contourf(x, y, z, 100, cmap=custom_cmap, antialiased=True)

# No need to manually set edgecolor properties - simplified approach
...
# Save with transparent background - explicitly set transparent=True
plt.savefig('public/images/contour_plot_background.png', 
            dpi=300, 
            bbox_inches='tight', 
            pad_inches=0.3,
            transparent=True,
...
print("Contour plot with ABSOLUTELY NO WHITE LINES generated successfully.")